  ...
  getDashboard(){
	...
	/*return new Promise(resolve => {
		...
		getClassData()
		{
				...
				this.http.get("http://preschool.weppsol.com/media/json_data/classes.json",{"headers":headers})
					.map(res => res.json())
					.subscribe(data => {
					// and save the data for later reference
					this.classdt = data.data[0].classes;
					resolve(this.classdt);
					});
				});
		...
		getDivisionData()
		{
				...
				this.http.get("http://preschool.weppsol.com/media/json_data/classes.json",{"headers":headers})
					...
					.subscribe(data => {
					// and save the data for later reference
					this.divdt = data.data[0].division;
					resolve(this.divdt);
					});
				});
		...
		getGenderData()
		{
				...
				this.http.get("http://preschool.weppsol.com/media/json_data/comman.json",{"headers":headers})
					.map(res => res.json())
					.subscribe(data => {
					// and save the data for later reference
					this.divdt = data.gender;
					resolve(this.divdt);
					});
				});
